    ...
    <div class="rounded-2xl p-8">
      ...
      <div class="mb-8 bg-gray-50 rounded-xl p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          ...
          <div>
            ...
            <select
              ...
              @change="applyFilters"
              class="w-full px-3 py-2 border text-gray-700 border-gray-300 font-bold rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
            >
          ...
          <div>
            ...
            >
              ...
              <option
                v-for="broker in uniqueBrokerages"
                :key="broker"
                :value="broker"
              >
                {{ broker }}
        ...
        <div class="flex flex-wrap gap-4 items-center justify-between mt-8">
          ...
          <div class="flex items-center">
            <select
              ...
              @change="applyFilters"
              class="px-3 py-2 border text-gray-700 border-gray-300 font-bold rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
            >
        ...
        <div class="text-sm text-gray-600 flex justify-end w-full">
          Mostrando {{ filteredStocks.length }} de
          {{ stocks.length }} resultados
        </div>
      </div>
      ...
      <div class="relative">
        <div class="overflow-hidden rounded-xl">
          ...
          >
            ...
            >
              <div
                class="relative bg-gradient-to-br from-white to-gray-50 rounded-xl p-6 border-2 border-gray-100 hover:border-orange-300 hover:shadow-lg transition-all duration-300 group h-full"
              >
        ...
        <div class="flex justify-center mt-6 space-x-2 overflow-x-auto pb-2">
          ...
          <div class="flex space-x-2">
            <button
              v-for="index in Math.min(
                5,
                Math.ceil(filteredStocks.length / visibleCards)
              )"
              :key="index"
          ...
          <div
            v-if="Math.ceil(filteredStocks.length / visibleCards) > 5"
...
const uniqueBrokerages = computed(() => {
  return [...new Set(stocks.value.map((stock) => stock.brokerage))].filter(
    Boolean
  );
});
...
const filteredStocks = computed(() => {
  ...
  if (filters.value.brokerage) {
    filtered = filtered.filter(
      (stock) => stock.brokerage === filters.value.brokerage
    );
  }

  if (filters.value.minScore > 0) {
    filtered = filtered.filter(
      (stock) => (stock.score || 0) >= filters.value.minScore
    );
  }
...
const maxSlides = computed(() => {
  return Math.max(
    0,
    Math.ceil(filteredStocks.value.length / visibleCards.value) - 1
  );
});
...
const buyCount = computed(() => {
  return filteredStocks.value.filter(
    (stock) => (stock.current_rating || stock.rating_to) === "Buy"
  ).length;
});

const holdCount = computed(() => {
  return filteredStocks.value.filter(
    (stock) => (stock.current_rating || stock.rating_to) === "Hold"
  ).length;
});
...
const highConfidenceCount = computed(() => {
  return filteredStocks.value.filter((stock) => (stock.confidence || 0) > 0.8)
    .length;
});
